(function (globalScope) {
    ...
    function buildHVACAnalysisPrompt(summary) {
        return [
            ...
            `- "periodBreakdown" is the most important field: it shows how the system behaved over time, bucketed ${summary && summary.breakdownGranularity === 'daily' ? 'by DAY (short span)' : summary && summary.breakdownGranularity === 'weekly' ? 'by WEEK' : summary && summary.breakdownGranularity === 'monthly' ? 'by MONTH (long span)' : 'by QUARTER (very long span)'}. Each entry is labeled with "${summary && summary.breakdownGranularity === 'daily' ? 'dayStart' : summary && summary.breakdownGranularity === 'weekly' ? 'weekStart' : summary && summary.breakdownGranularity === 'monthly' ? 'monthStart' : 'quarterStart'}". ALWAYS analyze trends over time here before drawing conclusions.`,
            '- Compare each period\'s runtime/cycle metrics against that same period\'s "avgOutdoorTemp". High runtime in a mild period is suspicious; high runtime in a hot period may be normal.',
            '- Window-wide aggregates (averages, "maxCoolingCycleMinutes", "coolingIntervalsTempRose", etc.) are summed/averaged across the WHOLE period and can be dominated by a single hot period. Do NOT assume an extreme reflects the current/most-recent state — attribute it to the period(s) it occurred in using "periodBreakdown".',
            '- Explicitly state whether any problem appears ongoing or whether it improved/resolved over the period (e.g. an early anomaly that normalized later).',
